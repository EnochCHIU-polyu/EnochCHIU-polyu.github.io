...
# Transaction Processing in Geth: Real Control Flow and Data Flow

## 0. Node Architecture: Two-Layer Model

Since The Merge in 2022, a full Ethereum node is not a single monolithic client. A complete node must run two cooperating software layers:

1. Execution Layer (EL)
2. Consensus Layer (CL)

These layers are separate processes, usually from different client projects, and they communicate through the Engine API.

### 0.1 Execution Layer (EL)
...
Geth is an Execution Layer client. Its role is to execute transactions and maintain Ethereum execution state.
...
- Validate execution payloads received from the Consensus Layer through Engine API calls.

In this repository, the EL responsibilities are spread across RPC APIs, txpool, EVM/state transition code, state processing, blockchain import validation, and Engine API handling.

### 0.2 Consensus Layer (CL)
...
The Consensus Layer decides which block is the canonical head, which checkpoints are safe/finalized, and which validator is responsible for proposing a block.

Primary responsibilities:
- Run Proof of Stake consensus.
- Manage validator duties, attestations, slots, and epochs.
- Execute fork choice and handle reorgs.
- Track head, safe head, and finalized checkpoint.
- Instruct the Execution Layer through the Engine API.

The CL does not execute EVM transactions itself. Instead, it asks the EL to build, execute, or validate execution payloads.

### 0.3 Engine API Bridge

The Engine API is the mandatory bridge between CL and EL.

Typical calls:
- `engine_forkchoiceUpdated*`: CL tells EL the current forkchoice head/safe/finalized view, optionally with payload attributes for building.
- `engine_getPayload*`: CL asks EL for a locally built execution payload.
- `engine_newPayload*`: CL submits an execution payload to EL for execution and validation.
